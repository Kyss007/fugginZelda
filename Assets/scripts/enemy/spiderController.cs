...
public class SpiderController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            // Airborne, keep feet at home positions
            for (int i = 0; i < legs.Length; i++)
            {
    ...
    IEnumerator PerformStep(int legIndex)
    {
        ...
        adaptedDuration = Mathf.Max(adaptedDuration, 0.01f); // prevent divide-by-zero
        ...
        while (elapsed < adaptedDuration)
        {
            ...
            float normalizedTime = Mathf.Clamp01(elapsed / adaptedDuration);
            legs[legIndex].stepProgress = normalizedTime;

            targetPosition = FindGroundPoint(legs[legIndex].worldHomePosition, out _);

            Vector3 currentPosition = Vector3.Lerp(startPosition, targetPosition, stepCurve.Evaluate(normalizedTime));
            currentPosition.y += Mathf.Sin(normalizedTime * Mathf.PI) * stepHeight;
        ...
        Vector3 finalPosition = FindGroundPoint(legs[legIndex].worldHomePosition, out _);
        if (!float.IsNaN(finalPosition.x) &&
            !float.IsNaN(finalPosition.y) &&
            !float.IsNaN(finalPosition.z))
        {
            legs[legIndex].footTarget.position = finalPosition;
        }
